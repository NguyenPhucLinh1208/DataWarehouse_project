...
from ScrapingPackage.scraper import enter_stock_code, get_new_financial_data


def main_report_new(date_now):
    ...
    for idx, stock_code in enumerate(stocks, 1):
        ...
        try:
            Balance = get_new_financial_data(driver, "Cân Đối Kế Toán", stock_code, date_now)
        ...
        try:
            Income = get_new_financial_data(driver, "Kết Quả Kinh Doanh", stock_code, date_now)
        ...
        # Lưu chuyển tiền tệ
        try:
            CashFlow = get_new_financial_data(driver, "Lưu Chuyển Tiền Tệ", stock_code, date_now)
        ...
        if idx % 5 == 0:
            Total_Balance.to_csv("/opt/Tasks/HistoricalData/Total_Balance.csv", index=False)
            Total_Income.to_csv("/opt/Tasks/HistoricalData/Total_Income.csv", index=False)
            Total_CashFlow.to_csv("/opt/Tasks/HistoricalData/Total_CashFlow.csv", index=False)
