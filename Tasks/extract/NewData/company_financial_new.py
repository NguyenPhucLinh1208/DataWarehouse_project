...
def main_report_new():
    ...
    for idx, stock_code in enumerate(stocks, 1):
        ...
        if idx % 5 == 0:
            Total_Balance.to_csv("/opt/Tasks/extract/NewData/Total_Balance_0.csv", index=False)
            Total_Income.to_csv("/opt/Tasks/extract/NewData/Total_Income_0.csv", index=False)
            Total_CashFlow.to_csv("/opt/Tasks/extract/NewData/Total_CashFlow_0.csv", index=False)
    ...
    with open("/opt/Tasks/extract/NewData/next_stocks_0", 'w') as file:
