...
def main_report_1():
    ...
    for idx, stock_code in enumerate(stocks, 1):
        ...
        if idx % 5 == 0:
            Total_Balance.to_csv("/opt/Tasks/extract/HistoricalData/Total_Balance_1.csv", index=False)
            Total_Income.to_csv("/opt/Tasks/extract/HistoricalData/Total_Income_1.csv", index=False)
            Total_CashFlow.to_csv("/opt/Tasks/extract/HistoricalData/Total_CashFlow_1.csv", index=False)
