...
def main_report():
    ...
    for idx, stock_code in enumerate(stocks, 1):
        ...
        if idx % 5 == 0:
            Total_Balance.to_csv("/opt/Tasks/HistoricalData/Total_Balance_0.csv", index=False)
            Total_Income.to_csv("/opt/Tasks/HistoricalData/Total_Income_0.csv", index=False)
            Total_CashFlow.to_csv("/opt/Tasks/HistoricalData/Total_CashFlow_0.csv", index=False)
