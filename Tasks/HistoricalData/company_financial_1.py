...
def main_report_1():
    ...
    for idx, stock_code in enumerate(stocks, 1):
        ...
        if idx % 5 == 0:
            Total_Balance.to_csv("/opt/Tasks/HistoricalData/Total_Balance1.csv", index=False)
            Total_Income.to_csv("/opt/Tasks/HistoricalData/Total_Income1.csv", index=False)
            Total_CashFlow.to_csv("/opt/Tasks/HistoricalData/Total_CashFlow1.csv", index=False)
